...
export const up = async (db: Kysely<any>): Promise<void> => {
  ...
  await db.schema
    ...
    .addColumn("trip_id", "uuid", (col) =>
      col.notNull().references("trip.id").onDelete("cascade")
    )
    .addColumn("destination_id", "uuid", (col) =>
      col.notNull().references("destination.id").onDelete("cascade")
    )
    .addPrimaryKeyConstraint("trip_destination_pkey", [
      "trip_id",
      "destination_id",
    ])
    .execute();
  ...
  await db.schema
    ...
    .addPrimaryKeyConstraint("trip_votes_pkey", [
      "trip_id",
      "destination_id",
      "user_id",
    ])
